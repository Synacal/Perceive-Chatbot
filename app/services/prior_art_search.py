...
async def search_documents(query: SearchQuery):
    ...
    #     FROM target.patents
    #     WHERE (title IS NOT NULL AND title <> '' AND abstract IS NOT NULL AND abstract <> '' AND claim_text IS NOT NULL AND claim_text <> '')
    #         AND ((title_abstract_tsvector @@ {ts_query})
    ...
    query = sql.SQL(
        """
            ...
            FROM ranked_patents
            WHERE rn <= 8490867
            AND (title IS NOT NULL AND title <> '' AND abstract IS NOT NULL AND abstract <> '' AND claim_text IS NOT NULL AND claim_text <> '')
            AND ((title_abstract_tsvector @@ {ts_query})
                OR (claims_tsvector @@ {ts_query}));
            """
    ).format(ts_query=ts_query)
